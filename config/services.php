...
return static function (ContainerConfigurator $container) {
    ...
    $services->set(CoverageCollector::class)
        ->arg('$enabled', '%dead_code.enabled%')
        ->arg('$samplingRate', '%dead_code.sampling_rate%')
        ->arg('$ignoredPaths', '%dead_code.ignored_paths%');

    $services->set(RedisStorage::class)
        ->arg('$dsn', '%dead_code.redis_dsn%')
        ->arg('$ttl', '%dead_code.cache_ttl%');

    $services->set(CoverageReporter::class)
        ->arg('$storage', service(RedisStorage::class));

    $services->set(CoverageListener::class)
        ->arg('$collector', service(CoverageCollector::class))
        ->arg('$storage', service(RedisStorage::class))
    ...
    $services->set(GenerateReportCommand::class)
        ->arg('$reporter', service(CoverageReporter::class))
        ->tag('console.command');
};
